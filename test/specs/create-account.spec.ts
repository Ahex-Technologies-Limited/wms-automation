...
import CreateAccountPage from '../pageobjects/create-account.page';
    ...
    describe('Create Account Functionality', function () {
        ...
        it('TC003 Verify that an error message should display for invalid email format', async function () {
            ...
            await expect(await CreateAccountPage.emailError.isDisplayed()).toBe(true);
            await expect(await CreateAccountPage.emailError.getText()).toContain('Please enter a valid email address');
        });
        ...
        it('TC004 Verify that an error message should display for short password', async function () {
            ...
            // Validate that password length error message is displayed
        ...
        it
        ('TC005 Verify that an error message should display for empty fields', async function () {
            await CreateAccountPage.register(
                '',
                '',
                '',
        ...
        it('TC006 Verify that it should show an error message for mismatched passwords', async function () {
            ...
            await expect(await CreateAccountPage.passwordError.getText()).toContain('Passwords do not match');
        });
        ...
        it('TC008 Verify it should show an error message for already registered email', async function () {
            ...
            const emailError = await $('body > app-root > app-register > div > div.register-right.relative > div > form > div:nth-child(3) > app-validation-error > div > small');
            expect(await emailError.isDisplayed()).toBe(true);
            expect(await emailError.getText()).toContain('Email already exists');
        });
